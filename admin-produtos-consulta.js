...
const DOM = {
    tabelaProdutos: document.getElementById('tabela-produtos-tbody')
}

let todosProdutos = [];

function renderizarProdutos(produtosParaExibir) {
    DOM.tabelaProdutos.innerHTML = '';

    if (produtosParaExibir.length === 0) {
        DOM.tabelaProdutos.innerHTML = `
            <tr data-elemento="linha-sem-produtos">
    ...
    produtosParaExibir.forEach(produto => {
        const HTML = `
            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                ...
                <td class="px-6 py-4 text-right">
                    <a href="admin-produtos-cadastro.html?id=${encodeURIComponent(produto._id)}"
                       data-elemento="botao-editar-produto"
                    ...
                    <button data-id="${produto._id}"
                            data-elemento="botao-excluir-produto"
        ...
        `;
        DOM.tabelaProdutos.insertAdjacentHTML('beforeend', HTML);
    });
}
...
DOM.tabelaProdutos.addEventListener('click', async (evento) => {
